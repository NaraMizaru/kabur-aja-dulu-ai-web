...
const RegisterPage = ({ onNavigate }) => {
  return (
    <div className="min-h-screen bg-[#0F111A] text-white font-sans antialiased p-8 lg:p-12 relative">
      ...
      <div className="flex items-center justify-center -mt-10 min-h-[70vh]">
        <div className="w-full max-w-md bg-[#171B26] border border-slate-800 p-10 rounded-[2.5rem] shadow-2xl">
          ...
          <p className="text-slate-400 mb-8">Silahkan daftar untuk memulai.</p>
          
          <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-300 ml-1">Nama Lengkap</label>
              <input type="text" placeholder="Username" className="w-full bg-[#0F111A] border border-slate-700 rounded-2xl px-5 py-4 focus:outline-none focus:border-[#00CFFF] transition-all" />
            </div>
            <div className="space-y-2">
          ...
          <p className="mt-8 text-center text-slate-400 text-sm">
            Sudah punya akun? <button onClick={() => onNavigate('login')} className="text-[#00CFFF] font-bold underline underline-offset-4 decoration-[#00CFFF]">Masuk</button>
          </p>
        </div>
      </div>
    </div>
